...
#' 
#' @title
#' The required sample size for testing a null hyphotesis for a single difference of variances
#' @description 
#' This function returns the minimum sample size required for testing a null hyphotesis regarding a single difference of variances
#' @details
#' We assume that it is of interest to test the following set of hyphotesis:
#' \deqn{H_0: S^2_1 - S^2_2 = 0 \ \ \ \ vs. \ \ \ \ H_a: S^2_1 - S^2_2 = D > 0 }
#' Note that the minimun sample size, restricted to the predefined power \eqn{\beta} and confidence \eqn{1-\alpha}, is defined by: 
#' \deqn{n = \frac{S^2}{\frac{D^2}{(z_{1-\alpha} + z_{\beta})^2}\frac{(N-3)(N-2)}{N(2N-7)}+\frac{S^2}{N}}}
#' Where \eqn{S^2=(S^4_1+S^4_2)DEFF}.
...
#' @examples 
#' ss4dS2H(N=100000, S2.1=100, S2.2=120, D=20)
#' ss4dS2H(N=100000, S2.1=100, S2.2=120, D=20, power = 0.9)
#' ss4dS2H(N=100000, S2.1=100, S2.2=120, D=20, plo=TRUE)
#' 
#' #############################
...
ss4dS2H = function(N, S2.1, S2.2, D, DEFF=1, conf=0.95, power=0.8, plot=FALSE){
  
  S2 = (S2.1^2 + S2.2^2)*DEFF
  Za = conf
  Zb = power 
  Z = qnorm(Za)+qnorm(Zb)
  n.hyp = S2/((D^2*(N-3)*(N-2)/(Z^2*N*(2*N-7))) + (S2/N))
  n.hyp = ceiling(n.hyp)
  ...
  if(plot == TRUE) {
    ...
    for(k in 1:length(nseq)){
      fseq=nseq[k]/N
      varseq=(1/fseq)*(1-fseq)*(2*N-7)*S2*(qnorm(Za)+qnorm(Zb))^2/((N-3)*(N-2))
      Dseq[k]=sqrt(varseq)
    }
