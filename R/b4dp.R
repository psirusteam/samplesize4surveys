...
#' @details
#' We note that the power is defined as: \deqn{1-\Phi(Z_{1-\alpha} - \frac{(D - (P_1 - P_2))}{\sqrt{\frac{DEFF}{n}(1-\frac{n}{N})(P_1Q_1+P_2Q_2)}})}
...
b4dp <- function(N, n, P1, P2, D, DEFF = 1, conf = 0.95, plot = FALSE){
  Q1 <- 1 - P1
  Q2 <- 1 - P2
  S2 <- (P1 * Q1 + P2 * Q2) * DEFF
  Za = qnorm(conf)
  f = n/N
  VAR = (1 / n) * (1 - f) * S2
  beta = 100 * (1 - pnorm(Za - ((D - (P1 - P2)) / sqrt(VAR))))
  ...
  if(plot == TRUE) {
    ...
    for(k in 1:length(nseq)){
      ...
      betaseq[k]=100*(1 - pnorm(Za - ((D - (P1 - P2)) / sqrt(varseq))))
    }
